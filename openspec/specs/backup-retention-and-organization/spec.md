# backup-retention-and-organization

## Purpose

Define how sequoia organizes, locates, and prunes the backups it creates during `sequoia install` and `sequoia uninstall`. Every backup MUST live under a single per-user root resolved via `os.UserConfigDir()`, partitioned by adapter ID and session timestamp, with a per-adapter retention cap of 5 most recent sessions enforced at the end of every successful install. Pre-existing scattered backups from prior sequoia versions MUST NOT be migrated; a TUI note SHALL inform the user that those backups remain at their original locations.
