# cli-version-resolution Specification

## Purpose

Defines how the Sequoia CLI resolves its build version for display. The `resolveVersion` function ensures the TUI Welcome screen and `version` command agree on the version string, even when built without ldflags.
